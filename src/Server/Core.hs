...
tmain = withSocketsDo $ do
  ...
  print $ "listening port " ++ show port
  forever $ do
    (handle, host, port) <- accept sock
    print $ "Accepted connection form host : " ++ host ++ " port : " ++ show port
      ...
      case words st of
        ...
        "trans":a:b:[] -> do
          let i = read a :: Int
              j = read b :: Int
          atomically $ do
            (i0,j0) <- readTVar clientPos
            orig@(Elem g b) <- readArray masterMaze (i0,j0)
            writeArray masterMaze (i0,j0) $ changeBody orig NoBody

            writeTVar clientPos (i,j)
            new <- readArray masterMaze (i,j)
            writeArray masterMaze (i,j) $ changeBody new b
           -- writeArray (i,j)
